...
test_that("bernoulli unconstrain works", {
  ...
  expect_equal(
    q,
    bernoulli$param_unconstrain_json(paste("{\"theta\":", as.character(x), "}"))
  )
})
...
test_that("param_constrain handles rng arguments", {
  ...
  expect_equal(
    3,
    length(full$param_constrain(c(1.2), include_gq = TRUE, rng = rng))
  )
  expect_equal(
    4,
    length(full$param_constrain(
      c(1.2),
      include_tp = TRUE,
      include_gq = TRUE,
      rng = rng
  ...
  expect_error(
    full$param_constrain(c(1.2), include_gq = TRUE),
    "rng must be provided"
  )

  expect_error(
    full$param_constrain(c(1.2, 1.2)),
    "Incorrect number of unconstrained parameters"
  )
})
...
test_that("constructor propagates errors", {
  expect_error(
    load_model("throw_data", include_data = FALSE),
    "find this text: datafails"
  )
})
...
test_that("param_constrain propagates errors", {
  ...
  expect_error(
    m1$param_constrain(c(1.2), include_tp = TRUE),
    "find this text: tpfails"
  )

  m2 <- load_model("throw_gq", include_data = FALSE)
  m2$param_constrain(c(1.2)) # no error
  expect_error(
    m2$param_constrain(c(1.2), include_gq = TRUE, rng = m2$new_rng(1234)),
